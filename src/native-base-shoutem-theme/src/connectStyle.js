...
import { ThemeConsumer } from './StyleProvider';

// Modern context for propagating parent style path to descendants
...
export default (
  ...
  return function wrapWithStyledComponent(WrappedComponent) {
    ...
    class StyledComponentInner extends React.Component {
      ...
      constructor(props) {
        ...
        const finalStyle = this.getFinalStyle(
          props,
          this.getContextFromProps(props),
          style,
          styleNames
        );
      ...
      getParentPath() {
        if (!this.props.__parentPath) {
          return [componentStyleName];
        } else {
          return [
            ...this.props.__parentPath,
            componentStyleName,
      ...
      UNSAFE_componentWillReceiveProps(nextProps, nextContext) {
        ...
        if (this.shouldRebuildStyle(nextProps, nextContext, styleNames)) {
          const finalStyle = this.getFinalStyle(
            nextProps,
            this.getContextFromProps(nextProps),
            style,
            styleNames
          );
      ...
      shouldRebuildStyle(nextProps, nextContext, styleNames) {
        return (
          ...
          nextProps.__theme !== this.props.__theme ||
          !_.isEqual(nextProps.__parentPath, this.props.__parentPath) ||
      ...
      resolveConnectedComponentStyle(props) {
        ...
        return this.resolveStyle(this.getContextFromProps(this.props), props, styleNames)
          .componentStyle;
      }
      ...
      render() {
        ...
        return (
          <ParentPathContext.Provider value={this.getParentPath()}>
            <WrappedComponent
              {...this.props}
              {...addedProps}
              style={style}
    ...
    function StyledComponentWithContexts(props) {
      return (
        <ThemeConsumer>
          {({ theme }) => (
            <ParentPathContext.Consumer>
              {parentPath => (
                <StyledComponentInner
                  {...props}
                  __theme={theme || Theme.getDefaultTheme()}
    ...
    // Prevent hoisting React-specific statics like contextType to a function component
    return hoistStatics(StyledComponentWithContexts, WrappedComponent, {
      contextType: true,
      contextTypes: true,
